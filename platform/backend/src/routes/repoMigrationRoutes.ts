...
/**
 * Track agent start - emits WebSocket AND saves to migration.progress
 * FIX: This ensures dashboard shows agents
 */
...
/**
 * Track agent completion - emits WebSocket AND saves to migration.progress
 * FIX: This ensures dashboard shows completion
 */
...
async function processMigrationAsync(migrationId: string, repoPath: string): Promise<void> {
  ...
  try {
    ...
    if (isGitUrl(repoPath)) {
      ...
      trackAgentStart(migration, 'git-clone');
      ...
      trackAgentComplete(migration, 'git-clone', `Repository cloned to ${actualRepoPath}`);
    } else {
    ...
    migration.updatedAt = new Date();
    trackAgentStart(migration, 'code-analyzer');
...
logging.level.org.hibernate.type.descriptor.sql.BasicBinder=TRACE`,
    ...
    trackAgentComplete(migration, 'code-analyzer', JSON.stringify(codeAnalyzerOutput));
    ...
    migration.updatedAt = new Date();
    trackAgentStart(migration, 'migration-planner');
    ...
    trackAgentComplete(migration, 'migration-planner', plannerOutput);
    ...
    migration.updatedAt = new Date();
    trackAgentStart(migration, 'service-generator');
    logger.info('⚙️ [SERVICE GENERATOR] Generating Spring Boot microservices ONE AT A TIME...');

    const outputDir = path.join(workspaceDir, 'output');
    await fs.ensureDir(outputDir);
    await fs.ensureDir(path.join(outputDir, 'backend'));

    const microservices = migrationPlan.microservices || [];
    const totalServices = microservices.length;

    emitAgentLog(migrationId, 'service-generator', 'info', `🤖 Generating ${totalServices} microservices (ONE AT A TIME to avoid token limits)`, {
      microservices: totalServices
    });

    // Store all service outputs for frontend display
    let allServiceOutputs = '';
    const generatedServices: string[] = [];
    const arkCodeExtractor = require('../services/arkCodeExtractor').default;

    // OPTION B: Generate services ONE AT A TIME (prevents token limit issues)
    for (let i = 0; i < microservices.length; i++) {
      const service = microservices[i];
      const serviceName = service.name;
      const progress = ((i + 1) / totalServices * 100).toFixed(0);

      logger.info(`📦 [${i + 1}/${totalServices}] Generating ${serviceName}... (${progress}%)`);
      emitAgentLog(migrationId, 'service-generator', 'info', `⚙️ [${i + 1}/${totalServices}] Generating ${serviceName}...`);

      try {
        // Create single-service plan for this service only
        const singleServicePlan = {
          ...migrationPlan,
          microservices: [service] // Only this service
        };

        // Call ARK service-generator for ONE service
        ...
        if (serviceGenResult.success && serviceGenResult.rawOutput) {
          const serviceOutput = serviceGenResult.rawOutput;
          allServiceOutputs += `\n\n## ${serviceName}\n\n${serviceOutput}\n\n`;

          logger.info(`✅ Generated ${serviceName} code (${serviceOutput.length} chars)`);
          emitAgentLog(migrationId, 'service-generator', 'info', `✅ Generated ${serviceName} (${serviceOutput.length} chars)`);

          // IMMEDIATELY extract and write this service to disk
          ...
          if (extractResult.success && extractResult.filesWritten > 0) {
            generatedServices.push(serviceName);
            logger.info(`✅ Wrote ${serviceName} to disk: ${extractResult.filesWritten} files`);
            emitAgentLog(migrationId, 'service-generator', 'info', `✅ ${serviceName}: ${extractResult.filesWritten} files written`);
          } else {
            logger.error(`❌ Failed to write ${serviceName}: ${extractResult.errors?.join(', ')}`);
            emitAgentLog(migrationId, 'service-generator', 'error', `❌ Failed to write ${serviceName}`);
          }
        } else {
          logger.error(`❌ Failed to generate ${serviceName}`);
          emitAgentLog(migrationId, 'service-generator', 'error', `❌ Failed to generate ${serviceName}`);
        }
      } catch (error: any) {
        logger.error(`❌ Error generating ${serviceName}:`, error);
        emitAgentLog(migrationId, 'service-generator', 'error', `❌ Error: ${error.message}`);
      }
    ...
    // Store combined output for frontend display
    (migration as any).serviceGeneratorSpecs = allServiceOutputs;
    logger.info(`📝 Generated ${generatedServices.length}/${totalServices} services successfully`);
    logger.info(`✅ Services written to disk: ${generatedServices.join(', ')}`);

    // Output with markdown for frontend display
    const serviceGenOutput = JSON.stringify({
      microservices: migrationPlan.microservices || [],
      generatedServices: generatedServices, // Track what was actually generated
      arkRawOutput: allServiceOutputs
    });

    trackAgentComplete(migration, 'service-generator', serviceGenOutput);
    logger.info('✅ [SERVICE GENERATOR] Complete');

    // Step 4: Frontend Migrator - Generate Angular micro-frontends ONE AT A TIME
    await new Promise(resolve => setTimeout(resolve, 1000));
    trackAgentStart(migration, 'frontend-migrator');
    logger.info('🎨 [FRONTEND MIGRATOR] Generating Angular micro-frontends ONE AT A TIME...');

    await fs.ensureDir(path.join(outputDir, 'frontend'));

    const microFrontends = migrationPlan.microFrontends || [];
    const totalMFEs = microFrontends.length;

    emitAgentLog(migrationId, 'frontend-migrator', 'info', `🤖 Generating ${totalMFEs} micro-frontends (ONE AT A TIME to avoid token limits)`, {
      microFrontends: totalMFEs
    });

    // Store all frontend outputs for frontend display
    let allFrontendOutputs = '';
    const generatedMFEs: string[] = [];

    // OPTION B: Generate MFEs ONE AT A TIME (prevents token limit issues)
    for (let i = 0; i < microFrontends.length; i++) {
      const mfe = microFrontends[i];
      const mfeName = mfe.name;
      const progress = ((i + 1) / totalMFEs * 100).toFixed(0);

      logger.info(`🎨 [${i + 1}/${totalMFEs}] Generating ${mfeName}... (${progress}%)`);
      emitAgentLog(migrationId, 'frontend-migrator', 'info', `🎨 [${i + 1}/${totalMFEs}] Generating ${mfeName}...`);

      try {
        // Create single-MFE plan for this MFE only
        const singleMFEPlan = {
          ...migrationPlan,
          microFrontends: [mfe] // Only this MFE
        };

        // Call ARK frontend-migrator for ONE MFE
        ...
        if (frontendGenResult.success && frontendGenResult.rawOutput) {
          const mfeOutput = frontendGenResult.rawOutput;
          allFrontendOutputs += `\n\n## ${mfeName}\n\n${mfeOutput}\n\n`;

          logger.info(`✅ Generated ${mfeName} code (${mfeOutput.length} chars)`);
          emitAgentLog(migrationId, 'frontend-migrator', 'info', `✅ Generated ${mfeName} (${mfeOutput.length} chars)`);

          // IMMEDIATELY extract and write this MFE to disk
          ...
          if (extractResult.success && extractResult.filesWritten > 0) {
            generatedMFEs.push(mfeName);
            logger.info(`✅ Wrote ${mfeName} to disk: ${extractResult.filesWritten} files`);
            emitAgentLog(migrationId, 'frontend-migrator', 'info', `✅ ${mfeName}: ${extractResult.filesWritten} files written`);
          } else {
            logger.error(`❌ Failed to write ${mfeName}: ${extractResult.errors?.join(', ')}`);
            emitAgentLog(migrationId, 'frontend-migrator', 'error', `❌ Failed to write ${mfeName}`);
          }
        } else {
          logger.error(`❌ Failed to generate ${mfeName}`);
          emitAgentLog(migrationId, 'frontend-migrator', 'error', `❌ Failed to generate ${mfeName}`);
        }
      } catch (error: any) {
        logger.error(`❌ Error generating ${mfeName}:`, error);
        emitAgentLog(migrationId, 'frontend-migrator', 'error', `❌ Error: ${error.message}`);
      }

      // Small delay between MFEs
      await new Promise(resolve => setTimeout(resolve, 500));
    }

    // Store combined output for frontend display
    (migration as any).frontendMigratorSpecs = allFrontendOutputs;
    logger.info(`📝 Generated ${generatedMFEs.length}/${totalMFEs} micro-frontends successfully`);
    logger.info(`✅ Micro-frontends written to disk: ${generatedMFEs.join(', ')}`);

    // Output with markdown for frontend display
    const frontendOutput = JSON.stringify({
      microFrontends: migrationPlan.microFrontends || [],
      generatedMFEs: generatedMFEs, // Track what was actually generated
      arkRawOutput: allFrontendOutputs
    });

    trackAgentComplete(migration, 'frontend-migrator', frontendOutput);
    ...
    await new Promise(resolve => setTimeout(resolve, 1000));
    trackAgentStart(migration, 'quality-validator');
    ...
    try {
      // STEP 1: Run build validation FIRST (mvn package, npm build)
      logger.info('🔨 [BUILD VALIDATOR] Running build validation (mvn package, npm build)...');
      emitAgentLog(migrationId, 'quality-validator', 'info', '🔨 Running build validation on generated code...');

      const buildValidationReport = await functionalValidator.validateMigration(migrationId);
      const buildValidationOutput = functionalValidator.generateReport(buildValidationReport);

      validationOutput += '## Build Validation Report\n\n';
      validationOutput += buildValidationOutput;
      validationOutput += '\n\n---\n\n';

      // Store validation report
      (migration as any).validationReport = buildValidationReport;

      // Check if build passed
      if (buildValidationReport.overall === 'fail') {
        logger.error('❌ [BUILD VALIDATOR] Build validation failed!');
        emitAgentLog(migrationId, 'quality-validator', 'error', '❌ Build validation failed - code does not compile');

        // Build failed - skip ARK validation and report failure
        qualityValidationPassed = false;
        validationOutput += '\n\n❌ **Build validation failed. Skipping AI validation.**\n\n';

        trackAgentComplete(migration, 'quality-validator', validationOutput);

        // Continue to test validators (they will generate recommendations based on failed build)
        ...
        logger.info('✅ [BUILD VALIDATOR] Build validation passed!');
        emitAgentLog(migrationId, 'quality-validator', 'info', '✅ Build validation passed - code compiles successfully');

        // STEP 2: Build passed - now run AI validation for quality/architecture/security
        logger.info('🤖 [AI VALIDATOR] Running AI-based quality validation...');
        emitAgentLog(migrationId, 'quality-validator', 'info', '🤖 Running AI quality validation...');
...
Generate a comprehensive quality validation report with pass/fail status.`;
        ...
        if (qualityValidatorResult.success && qualityValidatorResult.rawOutput) {
          validationOutput += '## AI Quality Validation Report\n\n';
          validationOutput += qualityValidatorResult.rawOutput;
          qualityValidationPassed = true; // Build passed and AI validation completed
          emitAgentLog(migrationId, 'quality-validator', 'info', '✅ AI quality validation complete');
        } else {
          throw new Error('Quality validation failed via ARK');
        }

        trackAgentComplete(migration, 'quality-validator', validationOutput);
        logger.info('✅ [QUALITY VALIDATOR] Complete (Build + AI validation passed)');
      }
      // End of else block (build passed)
...
`;
    ...
    await new Promise(resolve => setTimeout(resolve, 1000));
    trackAgentStart(migration, 'unit-test-validator');
...
- Verify network connectivity to ARK API`;
      ...
      trackAgentComplete(migration, 'unit-test-validator', unitTestOutput);
...
- Review backend logs for more details`;
    ...
    await new Promise(resolve => setTimeout(resolve, 1000));
    trackAgentStart(migration, 'integration-test-validator');
...
- Verify network connectivity to ARK API`;
      ...
      trackAgentComplete(migration, 'integration-test-validator', integrationTestOutput);
...
- Review backend logs for more details`;
    ...
    await new Promise(resolve => setTimeout(resolve, 1000));
    trackAgentStart(migration, 'e2e-test-validator');
...
- Verify network connectivity to ARK API`;
      ...
      trackAgentComplete(migration, 'e2e-test-validator', e2eTestOutput);
...
- Review backend logs for more details`;
    ...
    // ============================================================================
    // CODE ALREADY EXTRACTED - services/MFEs were written immediately after generation
    // ============================================================================
    logger.info('✅ [CODE GENERATION] Code was extracted during service/frontend generation (no duplicate extraction needed)');
    emitAgentLog(migrationId, 'e2e-test-validator', 'info', '✅ Code files already written to disk during generation phase');
    ...
    try {
      ...
      const dockerComposeConfig = {
        ...
        includeApiGateway: false  // Changed to false: API Gateway not generated by service-generator
      };
      ...
      await dockerComposeGenerator.generateStartupScript(outputDir, dockerComposeConfig);
    ...
    // OPTION C: VALIDATE GENERATED CODE BEFORE DEPLOYMENT
    logger.info('🔍 [VALIDATION] Validating generated code before deployment...');
    emitAgentLog(migrationId, 'container-deployer', 'info', '🔍 Validating generated services and frontends');

    const validationResults = {
      validServices: [] as string[],
      validMFEs: [] as string[],
      missingServices: [] as string[],
      missingMFEs: [] as string[],
      servicesWithIssues: [] as {name: string, issues: string[]}[],
      mfesWithIssues: [] as {name: string, issues: string[]}[]
    };

    // Validate Backend Services
    const expectedServices = migrationPlan.microservices || [];
    for (const service of expectedServices) {
      const serviceName = service.name;
      const servicePath = path.join(outputDir, 'backend', serviceName);
      const issues: string[] = [];

      // Check if service directory exists
      if (!fs.existsSync(servicePath)) {
        validationResults.missingServices.push(serviceName);
        logger.warn(`❌ Service ${serviceName} NOT FOUND at ${servicePath}`);
        emitAgentLog(migrationId, 'container-deployer', 'warn', `❌ ${serviceName} directory not found`);
        continue;
      }
      ...
      if (issues.length > 0) {
        validationResults.servicesWithIssues.push({name: serviceName, issues});
        logger.warn(`⚠️ Service ${serviceName} has issues: ${issues.join(', ')}`);
        emitAgentLog(migrationId, 'container-deployer', 'warn', `⚠️ ${serviceName}: ${issues.join(', ')}`);
      } else {
        validationResults.validServices.push(serviceName);
        logger.info(`✅ Service ${serviceName} validated successfully`);
        emitAgentLog(migrationId, 'container-deployer', 'info', `✅ ${serviceName} validated`);
      }
    }

    // Validate Frontend Micro-Frontends
    const expectedMFEs = migrationPlan.microFrontends || [];
    for (const mfe of expectedMFEs) {
      const mfeName = mfe.name;
      const mfePath = path.join(outputDir, 'frontend', mfeName);
      const issues: string[] = [];

      // Check if MFE directory exists
      if (!fs.existsSync(mfePath)) {
        validationResults.missingMFEs.push(mfeName);
        logger.warn(`❌ MFE ${mfeName} NOT FOUND at ${mfePath}`);
        emitAgentLog(migrationId, 'container-deployer', 'warn', `❌ ${mfeName} directory not found`);
        continue;
      }
      ...
      for (const file of requiredFiles) {
        const filePath = path.join(mfePath, file);
        if (!fs.existsSync(filePath)) {
          issues.push(`Missing ${file}`);
        }
      }
      ...
      if (issues.length > 0) {
        validationResults.mfesWithIssues.push({name: mfeName, issues});
        logger.warn(`⚠️ MFE ${mfeName} has issues: ${issues.join(', ')}`);
        emitAgentLog(migrationId, 'container-deployer', 'warn', `⚠️ ${mfeName}: ${issues.join(', ')}`);
      } else {
        validationResults.validMFEs.push(mfeName);
        logger.info(`✅ MFE ${mfeName} validated successfully`);
        emitAgentLog(migrationId, 'container-deployer', 'info', `✅ ${mfeName} validated`);
      }
    }
...
🔍 VALIDATION SUMMARY:
✅ Valid Services: ${validationResults.validServices.length}/${expectedServices.length} (${validationResults.validServices.join(', ') || 'none'})
✅ Valid MFEs: ${validationResults.validMFEs.length}/${expectedMFEs.length} (${validationResults.validMFEs.join(', ') || 'none'})
${validationResults.missingServices.length > 0 ? `❌ Missing Services: ${validationResults.missingServices.join(', ')}` : ''}
${validationResults.missingMFEs.length > 0 ? `❌ Missing MFEs: ${validationResults.missingMFEs.join(', ')}` : ''}
${validationResults.servicesWithIssues.length > 0 ? `⚠️ Services with issues: ${validationResults.servicesWithIssues.map(s => s.name).join(', ')}` : ''}
${validationResults.mfesWithIssues.length > 0 ? `⚠️ MFEs with issues: ${validationResults.mfesWithIssues.map(m => m.name).join(', ')}` : ''}
`;

    logger.info(validationSummary);
    emitAgentLog(migrationId, 'container-deployer', 'info', validationSummary);

    // Store validation results
    (migration as any).validationResults = validationResults;

    // Now proceed to deployment
    logger.info('✅ [DEPLOYMENT] Proceeding to container deployment with validated services');
    emitAgentLog(migrationId, 'container-deployer', 'info', '✅ Validation complete! Deploying validated services only');
    trackAgentStart(migration, 'container-deployer');
    ...
    emitAgentLog(migrationId, 'container-deployer', 'info', `📦 Building Docker images for ${validationResults.validServices.length} services + ${validationResults.validMFEs.length} frontends`);
    ...
    try {
      ...
      trackAgentComplete(migration, 'container-deployer', containerOutput);
      ...
      trackAgentComplete(migration, 'container-deployer', errorOutput);
    }
      ...
      if (validationReport.overall === 'fail') {
        ...
        setTimeout(async () => {
          try {
            ...
            const deploymentReport = migrationService.generateDeploymentReport(deployment);
            trackAgentComplete(migration, 'container-deployer', deploymentReport);
            ...
            logger.error('[CONTAINER DEPLOYER] Failed:', deployError);
            trackAgentComplete(migration, 'container-deployer', `❌ Deployment Failed: ${deployError.message}`);
          }
        }, 1000);
